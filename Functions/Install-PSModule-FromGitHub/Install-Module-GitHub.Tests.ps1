...
BeforeAll {
   ...
   Function Delete-PSModule-Dir() {
      ...
      If ($DeleteAllUsers) {
         $ModuleDir = Join-Path $Script:ScopeAllUsersDir $ModuleSubDir
         Remove-Item -Recurse -Force -EA SilentlyContinue -LiteralPath $ModuleDir
      }
      If ($DeleteCurrentUser) {
         $ModuleDir = Join-Path $Script:ScopeCurrentUserDir $ModuleSubDir
         Remove-Item -Recurse -Force -EA SilentlyContinue -LiteralPath $ModuleDir
      }
   }
   ...
   # PS Modul Installations-Dirs holen
   If ([String]::IsNullOrWhiteSpace($Script:ScopeAllUsersDir)) {
      $Script:ScopeAllUsersDir = . $InstallModuleGitHub_ps1 -GetScopeAllUsers
   }
   If ([String]::IsNullOrWhiteSpace($Script:ScopeCurrentUserDir)) {
      $Script:ScopeCurrentUserDir = . $InstallModuleGitHub_ps1 -GetScopeCurrentUser
   }
...
Describe 'Test Tester' {
   ...
   It 'Assert $Scope…Dir' {
      # $ScopeAllUsersDir | Should -Not -BeNullOrEmpty
      $Script:ScopeAllUsersDir | Should -Not -BeNullOrEmpty
      $ScopeCurrentUserDir | Should -Not -BeNullOrEmpty
   }
   ...
   It 'Modul ist nicht installiert, Setup in AllUsers Scope' {
      ...
      $V1ZipFile = $Script:ModuleVersions[([eModulVersion]::V100)].ZipFile
      $Res = . $InstallModuleGitHub_ps1 -InstallZip $V1ZipFile `
                              -ProposedDefaultScope AllUsers `
                              -InstallAllModules
      # $Res | Should -Be $True
      Test-Path -LiteralPath $something | Should -Be $true
   }
